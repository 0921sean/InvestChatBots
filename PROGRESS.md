...
- **2026-08-01 (저녁·관전 UX 재설계 결정 + fund 폴리시)** — 사용자와 대량 이터레이션. **결정 확정(빌드는 단계별 진행 예정)**.
  ...
  - **⏳ 남은 빌드 순서(Epic 새 이슈로 묶을 것)**: A)직위 산정+표시 B)Members UI(직위·full-box 테두리·per-bot 출근) C)섹터→종목 2단계 흐름+[결정]포맷+섹터태그+active-bots 신호 D)채팅 끊김 조사·수정+옛 테스트 피드 정리 E)섹터 박스 데스크 진행형 배선 F)루트 컷오버+배포 G)CONVENTION.md 갱신.
