...
- **로스터(내부→공개 ROT13)**: A→N 리서치·P→C 성장주·W→J 가치·H→U 실적·S→F 병목(독자·다중트렌드)·Q→D 타이밍+지수계좌·M→Z 거시브리핑·R→E 리스크(비투표).
...
- **보안·정체 은닉**: 오너 단일쿠키 인증(2FA 없음 — 실계좌 전 강화 필요) · 전략가 실명 전 히스토리 스크럽(재작성) · 봇 이니셜 ROT13 · 페르소나는 strategy_private(gitignore).
