...
def _s_sourcing_note(codes, names) -> str:
    ...
    try:
        ...
        sysp = ("너는 여러 산업의 공급망 병목에 밝은 기술 설명가다. 아래 회사들이 각자 속한 사슬"
                "(AI 인프라·전력망·원자력·방산·항공·바이오·우주 등 어떤 트렌드든)에서 무슨 부품·역할을 하고 "
...
_CURATION_SYS = (
    "너는 '공급망 초크포인트(병목)' 리서처다. 병목을 **기술의 최전선**에서 찾는다(#184) — 지금 막 "
    "대규모 전환이 일어나는 첨단 기술이 유발하는, '그게 없으면 전환 자체가 멈추는' 무명·상류 병목 기업"
    "('참치 말고 깻잎'). 항공 MRO·조선·물·방산 재무장 같은 범용 인프라의 만성 병목은 다루지 않는다 — "
    "수요 충격의 근원이 '최전선 기술 전환'인 것만.\n"
    "① 먼저 웹서치로 '지금 최전선 전환이 어디인지'(예: AI 인프라·CPO/광학·고급 메모리·첨단 패키징·"
    "AI발 전력/냉각 — 단 고정 목록이 아니라 지금 신호로 판단)와 그 사슬의 공급 부족 신호를 확인하라 — "
    "월별 매출 공시·증설 리드타임·백로그·ASP 급등·컨퍼런스 임원 발언 같은 1차 소스 우선. "
    "병목은 옮겨 다닌다 — 지난달 병목이 아니라 '다음' 병목을 짚어라.\n"
    "② 각 후보를 사이클 3유형 중 하나로 분류해 rationale 앞에 붙여라: [수요폭발](증설이 수요를 못 따라감) / "
    "[기술도약](신기술이 구기술 대체, 인증·전환 병목) / [공급제약](원자재·장비·산능 자체가 희소).\n"
    "③ 배제 룰 — 하나라도 걸리면 제외: 주영업이 아닌데 편승한 회사 · 증설이 쉬운 범용 소재 · "
    "이미 다 아는 대형주(주가에 반영) · 경쟁 분산으로 해자 없음 · 기관 관심 전무한 유동성 함정.\n"
    "④ 가점: 기관 커버리지 개시·지분 축적이 '이제 막' 시작된 종목.\n"
    "미국 상장 종목(티커)만. 아래 '제외 목록'에 있는 티커는 절대 내지 마라. "
    "이건 투자 권유가 아니라 '어디가 병목인지' 후보 조사다. "
    "출력은 오직 JSON 배열만: [{\"ticker\":\"...\",\"rationale\":\"[유형] 왜 병목인지 한글 한 줄\"}]. "
